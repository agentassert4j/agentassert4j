 ...
 * 模型编排的 toolCalls 被内部回路消费；本缓冲在调用前替换为观察回调、调用后按序
 * 交出完整编排 [A, B, …]，多轮累积进同一条记录（编排即回归单元）。
 *
 * <p>缓冲线程安全：stream 内部回路的工具执行可能发生在 reactor 线程。
 * {@link #decorate} 在 options 副本上替换为观察回调，业务对象零触碰；
 * 装饰失败静默返回原 Prompt（纯观察：绝不阻断业务，该次调用退回无观察装饰前的
 * 工具维不可见状态）。</p>
 *
 * @author axy-yxa
 * @since 2026-08-29
...
final class ToolInvocationObserver {
    ...
    /**
     * 替换观察回调：请求带工具定义时在 options 副本上逐个包装；无工具定义或
     * 装饰失败返回原 Prompt
     */
