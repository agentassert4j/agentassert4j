 ...
 *
 * <p>LangChain4j 的 ChatModel 层不携带会话概念（会话在 AiServices/ChatMemory 层），
 * SDK 用线程绑定的临时作用域补齐：</p>
 * <pre>{@code
 * try (RecordingContext ctx = RecordingContext.start("session-1")
 *         .withInvocationId("order-refund")) {
 *     assistant.chat("查一下订单 8841 的退款进度");
 * }
 * }</pre>
 * <p>作用域可嵌套，关闭时恢复外层。未声明时录制管道按既有退化策略处理
 * （sessionId 缺失的记录各自成独立会话）。仅在声明线程内生效——异步完成线程
 * 上的调用取不到上下文，需要标注的流式调用请在发起前完成声明。</p>
 *
 * @author axy-yxa
 * @since 2026-09-18
 */
