...
/**
 * Spring AI 2.x 类型到交互记录的字段映射。
 *
 * <p>映射契约与 springai1 一致（系统消息 → templateHash，末位用户消息 → userInput，
 * 工具定义 function 形状序列化）；差异点：2.x 的 Usage 接口直接暴露缓存读写 token，
 * 无需反射探测；思考 token 仍走反射尽力提取。</p>
 *
 * @author axy-yxa
 * @since 2026-08-27
 */
final class SpringAiRecordMapper {
    ...
    static final String SDK_VERSION = "agentassert4j-sdk-spring-ai2";
        ...
        if (response.getMetadata() != null) {
            ...
            if (usage != null) {
                ...
                if (usage.getCacheReadInputTokens() != null) {
                    record.setCacheReadTokens(usage.getCacheReadInputTokens().intValue());
                }
                if (usage.getCacheWriteInputTokens() != null) {
                    record.setCacheWriteTokens(usage.getCacheWriteInputTokens().intValue());
                }
                Integer reasoning = probeReasoningTokens(usage.getNativeUsage());
                if (reasoning != null) {
    ...
    /**
     * 思考 token 不在 Usage 接口上，属 provider 实现字段——反射尽力提取，
     * 取不到保持 null；SDK 不为读一个数引入具体 provider 依赖。
     */
    private static Integer probeReasoningTokens(Object nativeUsage) {
        if (nativeUsage == null) {
            return null;
        }
        Integer direct = invokeNumber(nativeUsage, "reasoningTokens");
        if (direct != null) {
            return direct;
        }
        Object details = invokeQuietly(nativeUsage, "completionTokensDetails");
        return details != null ? invokeNumber(details, "reasoningTokens") : null;
    }
