...
public class SqliteStorageRepository implements StorageRepository {
    ...
    @Override
    public synchronized void saveScenario(ScenarioDefinition definition) {
        // INSERT OR REPLACE：scenario_id 是业务主键，重复保存即声明更新
        String sql = "INSERT OR REPLACE INTO scenarios" + " (scenario_id, name, template_id, input_spec, assertions," + "  version, metadata, created_at, updated_at)" + " VALUES (?,?,?,?,?,?,?,?,?)";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            int i = 1;
            ps.setString(i++, definition.getScenarioId());
            ps.setString(i++, definition.getName());
            ps.setString(i++, definition.getTemplateId());
            ps.setString(i++, definition.getInputSpec());
            ps.setString(i++, definition.getAssertions());
            ps.setInt(i++, definition.getVersion());
            ps.setString(i++, definition.getMetadata());
            ps.setLong(i++, definition.getCreatedAt());
            ps.setLong(i++, definition.getUpdatedAt());
            ps.executeUpdate();
        } catch (SQLException e) {
            LOG.log(Level.SEVERE, "saveScenario failed", e);
            throw new StorageException("saveScenario", e);
        }
    }

    @Override
    public synchronized List<ScenarioDefinition> findScenarios() {
        List<ScenarioDefinition> result = new ArrayList<>();
        String sql = "SELECT * FROM scenarios ORDER BY scenario_id ASC";
        try (PreparedStatement ps = connection.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                ScenarioDefinition d = new ScenarioDefinition();
                d.setScenarioId(rs.getString("scenario_id"));
                d.setName(rs.getString("name"));
                d.setTemplateId(rs.getString("template_id"));
                d.setInputSpec(rs.getString("input_spec"));
                d.setAssertions(rs.getString("assertions"));
            ...
            LOG.log(Level.SEVERE, "findScenarios failed", e);
            throw new StorageException("findScenarios", e);
        }
        return result;
    }

    @Override
    public synchronized void saveScenarioRun(ScenarioRun run) {
        String sql = "INSERT INTO scenario_runs" + " (run_id, scenario_id, started_at, verdict, sample_count, pass_count, fail_count," + "  input_tokens, output_tokens, cache_read_tokens, cache_write_tokens, reasoning_tokens," + "  latency_ms, ttft_ms, cost_usd, report_ref, metadata)" + " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            int i = 1;
            ps.setString(i++, run.getRunId());
            ps.setString(i++, run.getScenarioId());
            ps.setLong(i++, run.getStartedAt());
            ps.setString(i++, run.getVerdict());
            ps.setInt(i++, run.getSampleCount());
            ps.setInt(i++, run.getPassCount());
            ps.setInt(i++, run.getFailCount());
            ps.setInt(i++, run.getInputTokens());
            ps.setInt(i++, run.getOutputTokens());
            setNullableInt(ps, i++, run.getCacheReadTokens());
            setNullableInt(ps, i++, run.getCacheWriteTokens());
            setNullableInt(ps, i++, run.getReasoningTokens());
            setNullableLong(ps, i++, run.getLatencyMs());
            setNullableLong(ps, i++, run.getTtftMs());
            setNullableDouble(ps, i++, run.getCostUsd());
            ps.setString(i++, run.getReportRef());
            ps.setString(i++, run.getMetadata());
            ps.executeUpdate();
        } catch (SQLException e) {
            LOG.log(Level.SEVERE, "saveScenarioRun failed", e);
            throw new StorageException("saveScenarioRun", e);
        }
    }

    @Override
    public synchronized List<ScenarioRun> findScenarioRuns(String scenarioId) {
        List<ScenarioRun> result = new ArrayList<>();
        String sql = "SELECT * FROM scenario_runs WHERE scenario_id = ? ORDER BY started_at ASC, run_id ASC";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    run.setScenarioId(rs.getString("scenario_id"));
                    run.setStartedAt(rs.getLong("started_at"));
                    run.setVerdict(rs.getString("verdict"));
                    run.setSampleCount(rs.getInt("sample_count"));
                    run.setPassCount(rs.getInt("pass_count"));
                    run.setFailCount(rs.getInt("fail_count"));
                    int inputTokens = rs.getInt("input_tokens");
                    run.setInputTokens(rs.wasNull() ? 0 : inputTokens);
                    int outputTokens = rs.getInt("output_tokens");
                    run.setOutputTokens(rs.wasNull() ? 0 : outputTokens);
                    int cacheRead = rs.getInt("cache_read_tokens");
                    run.setCacheReadTokens(rs.wasNull() ? null : cacheRead);
                    int cacheWrite = rs.getInt("cache_write_tokens");
                    run.setCacheWriteTokens(rs.wasNull() ? null : cacheWrite);
                    int reasoning = rs.getInt("reasoning_tokens");
                    run.setReasoningTokens(rs.wasNull() ? null : reasoning);
                    long latency = rs.getLong("latency_ms");
                    run.setLatencyMs(rs.wasNull() ? 0L : latency);
                    long ttft = rs.getLong("ttft_ms");
                    run.setTtftMs(rs.wasNull() ? null : ttft);
                    double cost = rs.getDouble("cost_usd");
                    run.setCostUsd(rs.wasNull() ? null : cost);
                    run.setReportRef(rs.getString("report_ref"));
                    run.setMetadata(rs.getString("metadata"));
                    result.add(run);
                }
            }
        } catch (SQLException e) {
            LOG.log(Level.SEVERE, "findScenarioRuns failed", e);
            throw new StorageException("findScenarioRuns", e);
        }
        return result;
    }
}
