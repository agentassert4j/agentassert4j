...
| `replay` | 全项目漂移检测+逐任务对齐（零 LLM 调用） | `--task`/`--invocation` 复合缩域、`--ci`、`--re-drive`、`--member-check`（成员判定：最新链对最近 N 条历史链，matched k of N 量化稳定性）、`--member-window <N\|all>`（样本窗解析阶梯：本次显式 > regression.memberSampleWindow 配置 > 内置 5；all=全历史考古仅限单次调用，配置默认只收有限整数）、`--full-chain`、`--max-total-calls`/`--max-total-tokens`、`--model`/`--endpoint`（本次运行的发射覆盖，requires --re-drive，胜过 llm.model/llm.endpoint 配置；emitter/报价/换模型告警如实反映）、`--dry-run`、`--json` |
...
| 2026-09-21 | round10 裁决落地：--model/--endpoint 发射覆盖旗标 + 两项池裁决关闭 | 维护者裁决（channel2/round10/merged-diagnosis-r10.md §六）：A 观测 raw 落档=不做（取证已闭环，redriveOf+modelResponse+结构化遥测；链式多轮无单一 wire 体）；B 已拒形状存储列=维持事件路径（事件是 reject 唯一真源，O(n) 扫描无迫近缺口）。**旗标实施**：replay 增 --model/--endpoint（requires --re-drive、空白值 E_USAGE、无 --re-drive E_USAGE 点名前提）——CliSupport.applyReDriveOverrides 单点并入配置后经 createLlmClient 同一装配路径生效（发射客户端单一装配点不分叉），executionConfig.model 单独携带（观测记录请求模型身份+换模型告警）；MCP re-drive 补 model/endpoint 参数（parity 守卫对齐）。**消费面结论=全库核实**：createLlmClient 唯一消费点在 ReplayCommand，框架自有 LLM 调用仅 replay --re-drive 一条链路——旗标面就两处，无其他面需要。OPERATIONS §6.3 配方改旗标优先（共享配置互踩的根因消解）。【测试钉】CliSupportLlmClientTest 覆盖生效钉 + CommandSmokeTest 双守卫钉 |
| 2026-09-23 | round11 备料期对码三项修复（v11 剧本 §九；维护者裁决「现在修，round11 复验修复」） | **解析器严格入口**：core 新增顶层 `JsonParseException`（非受检，社区主流形态——Jackson/Gson/org.json/minimal-json 均以非受检异常承载根因；结果型是 Rust/Go 惯例、Java 无语言设施）与 `RecursiveJsonParser.parseStrict`（失败抛根因：语法错误/嵌套超限/尾部多余字符/空白输入），宽松 `parse` 委托同一语法语义、异常照旧吞掉——R10 退化语义一行不动。**M1 补活**：record 深嵌套/合法非对象/语法垃圾三类拒收的 parseFailureDetail 改走严格入口，根因括注逐类可辨（r10 该修复因 parse 吞异常而不可达——死代码路径，制备期真 jar 探针实证）。**M2 补活**：ConfigLoader.loadAgentAssert4jConfig 加严格预检 + 根对象非 object 同路径披露，坏配置 stderr 渲染 `Config warning: config file … is unparsable (<根因>); built-in defaults are in effect`（r10 同为不可达死代码）。**rules 配置目录兜底修复**：RulesCommand 先加载主配置（lastMainConfigDirectory 由加载动作置位），任意 CWD 命中配置旁规则文件；Config warning 渲染抽 `CliSupport.renderConfigWarnings` 单源（openRepository 与 rules 共用）。exists 幂等路径 expectedVersion 不校验=MCP establish schema 契约明示 for force，非缺口（剧本 §九-①，台账留痕防重开）。【测试钉】RecursiveJsonParserTest parseStrict 七钉（含深度超限/尾垃圾/宽松入口回归）+ ConfigLoaderTest UnparsableDisclosure 四钉 + McpRecordIngestionTest 拒收根因两钉 + CommandSmokeTest rules 命中/坏配置警告两钉 |
