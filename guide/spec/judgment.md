...
| 2026-09-20 | 通道2 round8：维度 3/4 失配明细点名失败项 | 失配只报维度（content rules mismatch）不报失败项，修复者需回读规则声明自行对照——ComparisonResult 增四字段（missingRequiredKeywords/presentForbiddenKeywords/failedRegexPatterns/failedBehaviors，字典序排序保证 summary 可复现），compare() 在检查现场收集失败项身份（检查与收集同点，坍缩成布尔后不可恢复），summary 追加 (required missing: [..]; forbidden present: [..]) 与 (failed: [..]) 形态；BehaviorChecker.checkAll 随之无主码消费方删除（逐项 check 由比较器直调）；rules 命令任务规则尾行措辞精确化：违例逐条进任务报告并 exit 1，步级 verdict 不变（原「fold into CHANGED」与实际机制不符）。【测试钉】DeterministicComparatorTest.dimensions3and4_failuresItemizedWithNames |
| 2026-09-20 | 通道2 round8（维护者裁决）：dims 五键 Matched 后缀改名 | task-report/verify-report 共用的步骤度量片段（comparisonMetricsFragment）五键 toolSet/paramTypes/outputStructure/contentRules/behaviors → toolSetMatched/paramTypesMatched/outputStructureMatched/contentRulesMatched/behaviorsMatched——布尔语义恒为「该维是否一致（true=无差异）」，旧键名是名词非断言，纯 JSON 消费者需人类摘要行反推极性（round8 双宿主双点位名）；发布前窗口内完成，无外部消费者。发射点单源（片段本就单源），消费钉仅 CliSupportComparisonMetricsTest 两断言。【测试钉】CliSupportComparisonMetricsTest Matched 形态 |
