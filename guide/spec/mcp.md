...
| 2026-09-20 | round9 池裁决实施批：report 增 json 参数、record-show 寻址族对齐 | ①report 增 json（boolean）：diff=true 时缺省出人读巡检视图，json=true 改出 candidate-diff/1 机器报告——AI 消费者不经人读行解析即可拿到逐维候选差异，裁决建议回路（check 见候选 → report 看差异 → 建议 accept/reject）闭环；②record-show 增 session/invocation/index/latest 四参数（与 CLI record show 寻址族逐参对齐，CliMcpParityTest 守卫 CLI 选项必须被工具入参覆盖），recordId 不再是唯一必填——寻址校验收敛在命令层（E-USAGE/E-NO-DATA 包络复用），服务端不做前置人工判空；③rollback 工具描述补回执双身份披露句（executor=本次操作者、approvedBy=恢复版原审批人）。【测试钉】CliMcpParityTest 参数覆盖矩阵（新参数自动入覆盖面）+ JsonContractTest 寻址族与 candidate-diff 结构钉 |
