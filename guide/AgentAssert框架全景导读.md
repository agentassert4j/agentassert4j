...
对齐**：每个任务的最新链和次新链按调用点配对——基线执行过而新链没有的步骤，标「缺步骤」；新链多出来
的，标「新增步骤」；两边都有的逐对比指纹。配对主体是**调用点**（声明标签）——同一调用点两侧提示词
版本不同照常配对判定，行首注记「跨版本配对」（版本切换的行为对照含混杂变量，受控复核用 `--re-drive`
...
现场重建，写一份快照。永远和录制数据一致，永不撒谎。想让复核便宜一点的话，`--task`/`--invocation`
...
时附一行「身份未收编」警告。`--json` 让结果变成**逐行机器可读的报告**写到标准输出，人看的进度和诊断
...
| `agentassert4j approve` / `reject` | bare 裁决全部待裁决候选（渲染候选差异 → 转正/丢弃） | `--invocation <目标>` 缩域；`--approver <名字>`（缺省取系统用户） | 无候选出 2 |
