...
final class McpTools {
    ...
    static List<McpTool> tools(String db) {
        ...
        tools.add(McpTool.of("doctor", "Database health check: identity splits, unlabeled multi-step chains, malformed rule declarations, unestablished points — " + "each finding carries actionable advice. Read-only. Run this first when check refuses or a report looks wrong.", "{\"type\":\"object\",\"properties\":{},\"additionalProperties\":false}", args -> runCommand(capture -> {
        ...
        tools.add(McpTool.of("graph", "Runtime data-flow inspection: rebuilds the dependency graph from recorded interactions and shows who feeds whom " + "(tool arguments traced back to upstream responses) with cycle detection. Development-time survey; verdicts never consume the graph.", "{\"type\":\"object\",\"properties\":{},\"additionalProperties\":false}", args -> runCommand(capture -> {
        ...
        tools.add(McpTool.of("record", "Report one completed LLM interaction for storage — the entry ticket for non-Java stacks. " + "Send the raw OpenAI-compatible chat completion request and response JSON (strings); the server parses them into a stored interaction " + "with derived identity: a declared invocation label wins, otherwise the system prompt template anchors the point. " + "Idempotent: re-sending the same recordId (or the same response id) is stored once and reported as duplicate. " + "Declare invocation and taskKey to group calls into invocations and task chains; check and diff then work on them.", "{\"type\":\"object\",\"properties\":{" + "\"sessionId\":{\"type\":\"string\",\"description\":\"Recording session identity; task chains group within it\"}," + "\"request\":{\"type\":\"string\",\"description\":\"Raw OpenAI-compatible chat completion request JSON\"}," + "\"response\":{\"type\":\"string\",\"description\":\"Raw OpenAI-compatible chat completion response JSON\"}," + "\"invocation\":{\"type\":\"string\",\"description\":\"Declared invocation label (identity anchor; wins over template anchoring)\"}," + "\"taskKey\":{\"type\":\"string\",\"description\":\"Declared task key; groups chains across runs (stored in metadata)\"}," + "\"recordId\":{\"type\":\"string\",\"description\":\"Idempotency key; defaults to the response id, then a content hash\"}," + "\"timestamp\":{\"type\":\"integer\",\"description\":\"Epoch milliseconds of the call (defaults to now)\"}," + "\"latencyMs\":{\"type\":\"integer\",\"description\":\"Measured call latency in milliseconds, if known\"}," + "\"metadata\":{\"type\":\"string\",\"description\":\"Additional metadata as a JSON object string\"}}," + "\"required\":[\"sessionId\",\"request\",\"response\"],\"additionalProperties\":false}", args -> McpRecordIngestion.ingest(db, args)));
        tools.add(McpTool.of("establish", "Establish baselines for recorded invocations (idempotent, safe to re-run; force rebuilds existing baselines under the current judgment semantics). " + "Governance write: call after a trustworthy recorded run, typically on human instruction. " + "Agents declare themselves with approver like \"agent:<name>\"; the CLI audit command lists such writes.", "{\"type\":\"object\",\"properties\":{" + "\"invocation\":{\"type\":\"string\",\"description\":\"Only this invocation (defaults to all)\"}," + "\"approver\":{\"type\":\"string\",\"description\":\"Approver identity stamped on the baseline; agents use agent:<name>\"}," + "\"ref\":{\"type\":\"string\",\"description\":\"Code reference (e.g. a git commit) the baselines correspond to\"}," + "\"force\":{\"type\":\"boolean\",\"description\":\"Rebuild existing baselines under the current judgment semantics\"}}," + "\"additionalProperties\":false}", args -> runCommand(capture -> {
        ...
        tools.add(McpTool.of("accept", "Promote a pending candidate fingerprint (landed by check or diff on CHANGED) to the baseline; the previous baseline is archived " + "and restorable via the CLI rollback command. Governance write: call only when the human decides the new behavior is intended. " + "Agents declare themselves with approver like \"agent:<name>\".", "{\"type\":\"object\",\"properties\":{" + "\"invocation\":{\"type\":\"string\",\"description\":\"Invocation holding the candidate (defaults to all pending)\"}," + "\"approver\":{\"type\":\"string\",\"description\":\"Approver identity; agents use agent:<name>\"}," + "\"ref\":{\"type\":\"string\",\"description\":\"Code reference (e.g. a git commit) the promoted baseline corresponds to\"}}," + "\"additionalProperties\":false}", args -> runCommand(capture -> {
        ...
        tools.add(McpTool.of("reject", "Discard a pending candidate fingerprint and keep the current baseline. Governance write: call on the human decision to keep the old behavior. " + "Reverting the prompt change itself is git's job.", "{\"type\":\"object\",\"properties\":{" + "\"invocation\":{\"type\":\"string\",\"description\":\"Invocation holding the candidate (defaults to all pending)\"}}," + "\"additionalProperties\":false}", args -> runCommand(capture -> {
