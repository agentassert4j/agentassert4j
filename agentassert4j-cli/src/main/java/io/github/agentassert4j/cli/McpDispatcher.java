...
final class McpDispatcher implements StdioTransport.MessageHandler {
    ...
    private static final String INSTRUCTIONS = "Behavior regression for AI agents: record interactions first (starter SDK in-app, or the record tool), " + "establish baselines, then check/diff after prompt or model changes. " + "check/diff judge each task's latest chain against its approved baselines (accept updates those baselines); the local replay command without --ci instead pairs the latest chain with the previous chain — accept never flips existing chain history to PASS. " + "PASS compares structure and declared rules only — answer wording is not fingerprinted. " + "CHANGED findings land candidate fingerprints awaiting accept/reject. " + "Mutation tools (establish, accept, reject) write governance state: call them on human instruction " + "and declare agent identity via approver \"agent:<name>\".";
