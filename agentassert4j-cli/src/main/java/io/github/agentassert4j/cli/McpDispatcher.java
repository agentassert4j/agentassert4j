...
final class McpDispatcher implements StdioTransport.MessageHandler {
    ...
    private static final String INSTRUCTIONS = "Behavior regression for AI agents: record interactions first (starter SDK in-app, or the record tool), " + "establish baselines, then check/diff after prompt or model changes. " + "check/diff judge the latest execution of each invocation in each task's latest chain against its approved shape set (accept adds the candidate shape to the set, so any chain ending an approved shape rechecks green); the local replay command without --ci instead pairs the latest chain with the previous chain. " + "PASS compares structure and declared rules only — answer wording is not fingerprinted. " + "CHANGED findings land candidate fingerprints awaiting accept/reject. " + "Mutation tools (establish, accept, reject, rollback) write governance state: call them on human instruction " + "and declare agent identity via approver \"agent:<name>\".";
